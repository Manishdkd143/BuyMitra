...
import { getDistributorCancelledOrders } from "../../../services/distributor/orders.service";
...
const CancelledOrders = () => {
  ...
  const fetchOrders = async (page = 1, search = "") => {
    try {
      setLoading(true);
      const res = await getDistributorCancelledOrders({
        page,
        limit: 10,
        search,
      });
      ...
      if (res.success) {
        setOrders(res.data.cancelledOrders);
        setMeta(res.data.meta);
      }
    } catch (err) {
      console.error("Cancelled orders fetch failed", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 📊 Stats
  const stats = {
    totalCancelled: meta.totalOrders,
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Orders cancelled by customer or distributor
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-5">
            <p className="text-red-400 text-sm mb-1">Total Cancelled</p>
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
            <p className="text-blue-400 text-sm mb-1">Order Value</p>
            <p className="text-3xl font-bold text-white">
